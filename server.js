...
app.post("/api/promo/validate", async (req, res) => {
  try {
    const { code } = req.body;
    if (!code) return res.status(400).json({ valid: false, error: "ادخل الكود" });
    ...
    if (promo.usedCount >= promo.maxUses) return res.json({ valid: false, error: "الكود اتاستخدم الحد الأقصى" });
...
app.post("/api/orders", async (req, res) => {
  try {
    ...
    const order = await Order.create({
      ...
      totalBefore, totalAfter: totalBefore - discount
    });
...
// Seed (GET - no auth needed - run once)
app.get("/api/seed", async (req, res) => {
  try {
    ...
    await Product.insertMany([
      ...
      { name: "أيوب",    badge: "🏅 الكلاسيك", caffeine: 65, rating: 4.6, image: "ayoub.png",   status: "available", story: "اشرب الصبر واستمتع بالفرج ❤️ — سيدنا ايوب صبر وربنا فرجها عليه.", specs: "درجة التحميص: وسط | أنواع البن: برازيلي + روبوستا", sizes: [{weight:"100 جرام", price:75}, {weight:"500 جرام", price:350}] },
      { name: "ع السكة", badge: "🚗 للطريق",   caffeine: 80, rating: 4.7, image: "sikka.png",   status: "available", story: "استمتع بالرحله... السكه طويله 🚗 — توليفة للتركيز في رحلتك.", specs: "درجة التحميص: وسط | أنواع البن: برازيلي + كولومبي + اثيوبي", sizes: [{weight:"100 جرام", price:65}, {weight:"500 جرام", price:300}] },
      { name: "الجدع",   badge: "💪 للجدعان", caffeine: 50, rating: 4.5, image: "gad3.png",    status: "available", story: "توليفة الجدع... لكل راجل جدع 😉 — مش لأي حد.", specs: "درجة التحميص: وسط | أنواع البن: يمني + كولومبي", sizes: [{weight:"100 جرام", price:55}, {weight:"500 جرام", price:250}] },
